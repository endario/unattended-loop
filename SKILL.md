...
- [ ] **No active debugger to preserve**: any locally-running services the loop will touch must have no IDE-attached debugger with active breakpoints or state worth keeping. **The loop is pre-approved to `--force` restart IDE-classified services** per `loop.md` §Local-ops greenlight (no on-the-spot approver is available, and the IDE-flag heuristic is often stale or a classifier false positive); a live debug session left on a touched service will lose its state, and the loop will not pause on a breakpoint — it will hang or skip. Opt-out: quit the IDE debug session before launching, OR list the specific service in the brief's `## Off-limits without user input` to carve it out of the force-restart pre-approval. (If the project's `CLAUDE.md` has a local-services detection recipe, consult it.)
